...
from monai.transforms import (
    ...
    RandDeformGrid, RandGridDistortion, 
...
def generate_simulated_stacks(
    ...
    elastic_max_displacement: float = 2.0,
    grid_distortion_prob: float = 0.5,
    grid_distortion_max_displacement: float = 1.5,
    grid_distortion_spacing: Tuple[int, int, int] = (20, 20, 20),
):
    """
    ...
    Args:
        ...
        elastic_max_displacement (float): Maximum displacement for elastic deformation.
        grid_distortion_prob (float): Probability of applying grid distortion.
        grid_distortion_max_displacement (float): Maximum displacement for grid distortion.
        grid_distortion_spacing (tuple): Spacing for grid distortion control points.
    """
    ...
    def apply_nonlinear_deformation(volume: np.ndarray) -> np.ndarray:
        """Apply random nonlinear deformation to simulate underlying MRI changes."""
        if not enable_nonlinear or np.random.random() > nonlinear_prob:
            return volume
            
        # Use a simple approach with random local deformations
        # Create a deformation field by adding small random displacements
        deformed_vol = volume.copy().astype(np.float32)
        
        # Apply random local deformations in small regions
        num_deformations = np.random.randint(5, 15)  # Random number of deformation regions
        
        for _ in range(num_deformations):
            # Random region center
            center_x = np.random.randint(10, volume.shape[0] - 10)
            center_y = np.random.randint(10, volume.shape[1] - 10)
            center_z = np.random.randint(5, volume.shape[2] - 5)
            
            # Random region size
            region_size = np.random.randint(5, 20)
            
            # Random displacement magnitude
            max_disp = elastic_max_displacement
            disp_x = np.random.uniform(-max_disp, max_disp)
            disp_y = np.random.uniform(-max_disp, max_disp)
            disp_z = np.random.uniform(-max_disp * 0.5, max_disp * 0.5)
            
            # Apply displacement in the region
            start_x = max(0, center_x - region_size)
            end_x = min(volume.shape[0], center_x + region_size)
            start_y = max(0, center_y - region_size)
            end_y = min(volume.shape[1], center_y + region_size)
            start_z = max(0, center_z - region_size // 2)
            end_z = min(volume.shape[2], center_z + region_size // 2)
            
            # Create displacement field with Gaussian falloff
            for x in range(start_x, end_x):
                for y in range(start_y, end_y):
                    for z in range(start_z, end_z):
                        ...
                        # Gaussian falloff
                        weight = np.exp(-(dist**2) / (2 * (region_size/3)**2))
                        
                        # Apply displacement with weight
                        if weight > 0.1:  # Only apply if weight is significant
                            # Interpolate to get new position
                            new_x = x + disp_x * weight
                            new_y = y + disp_y * weight
                            new_z = z + disp_z * weight
    ...
    for i in range(n_stacks):
        ...
        deformed_vol = apply_nonlinear_deformation(vol.copy())
        ...
        metadata = {
            ...
            'elastic_max_displacement': elastic_max_displacement,
            'grid_distortion_prob': grid_distortion_prob,
            'grid_distortion_max_displacement': grid_distortion_max_displacement,
            'grid_distortion_spacing': list(grid_distortion_spacing),
        }
...
if __name__ == "__main__":
    ...
    parser.add_argument("--elastic-max-displacement", type=float, default=2.0, help="Maximum displacement for elastic deformation")
    parser.add_argument("--grid-distortion-prob", type=float, default=0.5, help="Probability of applying grid distortion")
    parser.add_argument("--grid-distortion-max-displacement", type=float, default=1.5, help="Maximum displacement for grid distortion")
    parser.add_argument("--grid-distortion-spacing", type=int, nargs=3, default=[20, 20, 20], help="Spacing for grid distortion control points (x y z)")
    ...
    generate_simulated_stacks(
        ...
        elastic_max_displacement=args.elastic_max_displacement,
        grid_distortion_prob=args.grid_distortion_prob,
        grid_distortion_max_displacement=args.grid_distortion_max_displacement,
        grid_distortion_spacing=tuple(args.grid_distortion_spacing)
    )