    ...
    class SliceAcqAdjointFunction(Function):
        ...
        def forward(
            ...
            if equalize:
                ...
                vol = vol
                vol_weight = torch.empty(0, device=slices.device)
            ctx.res_slice = res_slice
            ctx.interp_psf = interp_psf
        ...
        def backward(ctx, grad_vol):
            ...
            if equalize:
                ...
                transforms, psf, slices, slices_mask, vol_mask = saved
                vol = vol_weight = torch.empty(0, device=grad_vol.device)
