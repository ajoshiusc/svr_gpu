...
def run_svort(
    ...
    # Persist SVoRT outputs (transforms, full stacks, and volumes) to SVR_TEMP_DIR
    try:
        temp_dir = os.environ.get("SVR_TEMP_DIR")
        if temp_dir:
            svort_dir = os.path.join(temp_dir, "svort")
            os.makedirs(svort_dir, exist_ok=True)
            # save transforms_out
            for j, t in enumerate(transforms_out):
                try:
                    mat = t.matrix().detach().cpu().numpy()
                    np.save(os.path.join(svort_dir, f"transforms_out_{j}.npy"), mat)
                except Exception:
                    pass
            # save full stacks if available
            if "ss_svort_full" in locals():
                for j, s in enumerate(ss_svort_full):
                    try:
                        s.save(os.path.join(svort_dir, f"ss_svort_full_{j}.nii.gz"))
                    except Exception:
                        pass
            if "ss_stack_full" in locals():
                for j, s in enumerate(ss_stack_full):
                    try:
                        s.save(os.path.join(svort_dir, f"ss_stack_full_{j}.nii.gz"))
                    except Exception:
                        pass
            # save evaluated volume if present
            if "volume" in locals() and hasattr(volume, "save"):
                try:
                    volume.save(os.path.join(svort_dir, "svort_volume.nii.gz"))
                except Exception:
                    pass
            logging.info("Persisted SVoRT outputs to %s", svort_dir)
