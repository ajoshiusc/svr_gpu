...
def build_monaifbs_net(device):
    ...
    # Use MONAI 0.3.0 DynUNet architecture for checkpoint compatibility
    try:
        from .dynunet_monai_0_3_0 import DynUNet
        logging.info("Using MONAI 0.3.0 DynUNet architecture for checkpoint compatibility")
    except ImportError as e:
        logging.error(f"Failed to import MONAI 0.3.0 DynUNet: {e}")
        raise ImportError("monai needs to be installed in order to use monaifbs")
    ...
    # Load checkpoint
    checkpoint = torch.load(get_monaifbs_checkpoint(), map_location=device, weights_only=False)
    state_dict = checkpoint["net"]
    
    # Load with the compatible MONAI 0.3.0 architecture - should work perfectly
    missing_keys, unexpected_keys = net.load_state_dict(state_dict, strict=False)
    
    if not missing_keys and not unexpected_keys:
        logging.info("Checkpoint loaded successfully with all keys matched!")
    else:
        if missing_keys:
