...
def id_block(X, f):
    ...
    X = Activation("relu")(X)
    X = Conv3D(
        filters=f, kernel_size=1, kernel_initializer="he_uniform", padding="same"
    )(X)
...
def model_architecture():
    ...
    from keras.layers import GlobalAveragePooling3D, Dense, Dropout

    globalAP = GlobalAveragePooling3D()(conv_block2048)
    dense1 = Dense(units=256, activation="relu", kernel_initializer="he_uniform")(
        globalAP
    )
