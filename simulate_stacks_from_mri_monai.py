...
def generate_simulated_stacks(
    ...
    with random in-plane displacement per slice, ensuring the output stack 
    correctly aligns with the input volume.
    ...
    for i in range(n_stacks):
        ...
        for group in groups:
            ...
            for s_acq in group:
                ...
                # Create temporary image for resampling
                temp_img = nib.Nifti1Image(vol, affine)
                slice_img = resample_from_to(temp_img, (slice_target_shape, slice_affine), 
                                           order=1, cval=float(vol.min()))
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Generate simulated stacks from a 3D MRI volume using MONAI.")
